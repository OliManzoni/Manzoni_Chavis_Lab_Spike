...
T = {
    ...
    "align_method": {"Français": "Méthode d'alignement des axes :", "English": "Axis alignment method:"},
    "align_sweep": {"Français": "Par numéro de Sweep", "English": "By Sweep number"},
    "align_round": {"Français": "Par valeur de courant arrondie à l'entier (pA)", "English": "By current rounded to the nearest integer (pA)"},
...
if not uploaded_files:
    ...
    for f in uploaded_files:
        f.seek(0) # Réinitialisation du curseur pour Streamlit
        try:
            df = pd.read_csv(f)
    ...
    if df_list:
        ...
        if not all(col in master_df.columns for col in required_cols):
            ...
            # --- APPLICATION DU CHOIX D'ALIGNEMENT ---
            if alignment_choice == T["align_sweep"][lang]:
                # Option A : Groupement par numéro de Sweep
                # L'axe X conserve la valeur moyenne du courant (convertie en pA pour l'affichage uniforme)
                master_df['X_Axis'] = master_df['I_inj'] * 1000
                ...
                x_label = "Courant Injecté (pA) [Moyenne par Sweep]" if lang == "Français" else "Injected Current (pA) [Mean per Sweep]"
            else:
                # Option B : Groupement par valeur arrondie à l'entier le plus proche (pA)
                # On convertit en pA (x1000) et on arrondit à l'entier le plus proche (.round(0))
                master_df['I_inj_pA_Rounded'] = (master_df['I_inj'] * 1000).round(0)
                stats_df = master_df.groupby('I_inj_pA_Rounded').agg(
                    X_Value=('I_inj_pA_Rounded', 'first'),
                    N_Cells=('Cell_ID', 'nunique'),
                    Nb_Spikes_Mean=('Nb_Spikes', 'mean'),
                    Nb_Spikes_SEM=('Nb_Spikes', 'sem'),
                    V_steady_Mean=('V_steady', 'mean'),
            ...
            # Nettoyage des NaN pour le traçage
            ...
            st.success(f"✅ {cell_count} " + ("cellules fusionnées et alignées." if lang == "Français" else "cells merged and aligned."))
            ...
            with tab1:
                ...
                # Filtrage des courants positifs pour la courbe f-I
                stats_depol = stats_df[stats_df['X_Value'] >= -1e-3]
            ...
            with tab2:
                ...
                st.download_button(
                    ...
                    data=csv_bytes,
                    file_name="Averaged_Population_Results.csv",
                    mime="text/csv",
